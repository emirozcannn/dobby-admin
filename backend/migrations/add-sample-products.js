...
const addSampleProducts = async () => {
  ...
  try {
    ...
    const companyResult = await client.query('SELECT id FROM companies WHERE email = $1', ['admin@dobby.com']);
    const companyId = companyResult.rows[0].id;

    const categoriesResult = await client.query('SELECT id, name FROM categories WHERE company_id = $1', [companyId]);
    const categories = {};
    ...
    const products = [
      { name: 'Türk Kahvesi', price: 25, cost: 8, category: 'Kahve', description: 'Geleneksel Türk kahvesi' },
      { name: 'Cappuccino', price: 35, cost: 12, category: 'Kahve', description: 'İtalyan cappuccino' },
      ...
      { name: 'Bitki Çayı', price: 20, cost: 5, category: 'Çay', description: 'Karışık bitki çayı' },
      ...
      { name: 'Portakal Suyu', price: 18, cost: 7, category: 'İçecek', description: 'Taze sıkılmış' },
      
      { name: 'Cheesecake', price: 45, cost: 20, category: 'Tatlı', description: 'New York cheesecake' },
    ...
    for (const product of products) {
      ...
      if (categoryId) {
        await client.query(`
          INSERT INTO master_products (company_id, category_id, name, description, base_price, cost_price)
          VALUES ($1, $2, $3, $4, $5, $6)
